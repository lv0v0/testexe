...
public class UserDaoImpl implements UserDao {
    ...
    public void add(User user) {
        //1.定义sql
        String sql = "insert into loginuser values(?,?,?,?,?,?,null,null)";
        //2.执行sql
    ...
    public void update(User user) {
        String sql = "update loginuser set name = ?,gender = ? ,age = ? , address = ? , qq = ?, email = ? where id = ?";
    ...
    public int findTotalCount(Map<String, String[]> condition) {
        //1.定义模板初始化sql
        String sql = "select count(*) from loginuser where 1 = 1 ";
    ...
    public List<User> findByPage(int start, int rows, Map<String, String[]> condition) {
        String sql = "select * from loginuser  where 1 = 1 ";
